...
All your custom *message handlers* need to be registered on a *message pump*. This "pump" is an Arcus Messaging-provided service that retrieves the Azure Service Bus messages for you, and "pumps" them to the right *message handler*.
...
{
    // The amount of messages that will be retrieved in one batch from Azure Service Bus.
    // (default: 1).
    options.MaxMessagesPerBatch = 5;

    // Specifies the amount of messages that will be eagerly requested during processing.
    // Setting the PrefetchCount to a value higher then the MaxMessagesPerBatch value helps 
    ...
    {
        // The maximum number of messages to retrieve in a single Azure Service Bus session
        // The total number of messages = MaxConcurrentSessions x MaxMessagesPerSession
        // (default: 1).
        options.MaxMessagesPerSession = 5;
